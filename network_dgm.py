...
# U-Net Shape Discriminator : Blur Detection
# Input: generated image / ground truth 
# Output: mask
class MaskAwareDiscriminator(nn.Module):
    def __init__(self, opt):
        super(MaskAwareDiscriminator, self).__init__()
        
        in_channels = 3 # No Mask
        out_channels = 1 # BW Mask
        
        self.same1 = Conv2dLayer(in_channels, opt.latent_channels, 7, 1, 3, 
                                    pad_type = opt.pad_type, activation = opt.activation, norm = opt.norm, sn = True)

        # Down sampling
        self.down1 = Conv2dLayer(opt.latent_channels, 2*opt.latent_channels, 3, 2, 1, 
                                    pad_type = opt.pad_type, activation = opt.activation, norm = opt.norm, sn = True)
        self.down2 = Conv2dLayer(2*opt.latent_channels, 4*opt.latent_channels, 3, 2, 1, 
                                    pad_type = opt.pad_type, activation = opt.activation, norm = opt.norm, sn = True)
        self.down3 = Conv2dLayer(4*opt.latent_channels, 8*opt.latent_channels, 3, 2, 1, 
                                    pad_type = opt.pad_type, activation = opt.activation, norm = opt.norm, sn = True)
        
        # Up sampling
        self.up1 = TransposeConv2dLayer(8*opt.latent_channels, 4*opt.latent_channels, 3, 1, 1, 
                                        pad_type = opt.pad_type, activation = opt.activation, norm = opt.norm, sn = True, scale_factor = 2)
        self.up2 = TransposeConv2dLayer(8*opt.latent_channels, 2*opt.latent_channels, 3, 1, 1, 
                                        pad_type = opt.pad_type, activation = opt.activation, norm = opt.norm, sn = True, scale_factor = 2)
        self.up3 = TransposeConv2dLayer(4*opt.latent_channels, opt.latent_channels, 3, 1, 1, 
                                        pad_type = opt.pad_type, activation = opt.activation, norm = opt.norm, sn = True, scale_factor = 2)
    ...
    def forward(self, img):                         # img : [B, 3, 512, 512]
        x1 = self.same1(img)                        # x1 : [B, 64, 512, 512]
        
        x2 = self.down1(x1)                         # x2 : [B, 128, 256, 256]
        x3 = self.down2(x2)                         # x3 : [B, 256, 128, 128]
        x4 = self.down3(x3)                         # x4 : [B, 512, 64, 64]
        
        x5 = self.up1(x4)                           # x5 : [B, 256, 128, 128]
        x6 = self.up2(torch.cat([x3, x5], dim=1))   # x6 : [B, 128, 256, 256]
        x7 = self.up3(torch.cat([x2, x6], dim=1))   # x7 : [B, 64, 512, 512]
        
        x8 = self.same2(torch.cat([x1, x7], dim=1)) # x8 : [B, 1, 512, 512]
        return x8
